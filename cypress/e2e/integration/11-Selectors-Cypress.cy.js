...
describe('Cypress Selectors', () => {
    ...
    it('Cypress Selectors | first, last, eq', () => {
        ...
        cy.get('div.cards').last()
            .should('be.visible')
            .and('have.length', 1)
            .and('have.text', 'Project - Front-end 10')

        cy.get('div.cards').eq(7)
            .should('be.visible')
            .and('have.length', 1)
    ...
    it('Cypress Selectors | next, prev', () => {
        ...
        // Locate and verify the text of the second card from the end (card 19)
        cy.get('div.cards').last()
            .prev()
    ...
    it('Cypress Selectors | find, children, parent', () => {
        ...
        cy.get('a[href*="facebook"]').parent()
            .should('have.class', 'Footer_socials__7h4n1').parent()
            .should('have.class', 'has-text-centered').parent()
            .should('have.class', 'Footer_footer__kXlYa').parent()
            .should('have.class', 'App').parent()
            .should('have.id', 'root')


        cy.get('a[href*="facebook"]').parents().should('have.length', 7)
        cy.get('a[href*="facebook"]').parent().next().children()
